...
def generate_fit_curve_data( plot_xdata, plot_fit2_grad, plot_fit2_start, plot_fit2_end, plot_ydata, linestyle, dotstyle, y_errors=None, initial_guess=None, fit_function=None, extend_start=0, extend_end=0):
    """
    ...
    if fit_funktion == True and variabeln_fit == 1:
        ...
        fit2_x = np.linspace(min(plotX_xdata)+ extend_start , max(plotX_xdata)+ extend_end, 1000)
...
def cvsimport(file, delimiter, skip_header=True):
...
anzahl_messungen = 1552  # Kann leicht angepasst werden
skalierung =   1  # Skalierungsfaktor für die Ergebnisse (z.B. 1000 für mPa·s)
ergebnis_einheit = r'\degrees'  # Einheit des Ergebnisses no latex needed
ergebnis_name = r'Temperatur des Wassers'  # Name des Ergebnisses
...
daten = cvsimport('PW10_V2.csv', ',', True)
t = np.float64(daten[:, 4])
T = np.float64(daten[:, 6])
print(t, T)
error_t = [0] * anzahl_messungen


error_T = [] 
error_T = unsicherheiten_Multimeter(T, 0.012, 0.1, 3)

# Variablen und deren Werte sammeln für text 
variablen_werte = {
    'Temperatur' : T,
    'Zeit' : t,
    'Fehler Temperatur' : error_T,
    'Fehler Zeit' : error_t
...
messwerte = [
    [T[i], t[i]] for i in range(anzahl_messungen) 
]
# Fehler für jede Variable
fehler = [
    [error_T[i], error_t[i]] for i in range(anzahl_messungen)
]
...
plot_xlabel = r'Zeit in Sekunden'                 # Beschriftung der x-Achse
plot_ylabel = r'Temperatur in $^{\circ}$C '      # Beschriftung der y-Achse
plot_titel  = r'' # Titel des Plots
plot_error = False                # Fehlerbalken im Plot anzeigen
plot_xdata = t  # x-Achse: wenn Messungezahal, dann : Messungsnummern if False dann ergebnisse
...
fit_funktion = False    # Hier die gewünschte Fit-Funktion angeben if true curve_fit if false polyfit
...
if plot_fit1:
    ...
    plot_fit1_xdata = t
    plot_fit1_ydata = T
    plot_fit1_extend_start = 0
    plot_fit1_extend_end = 50


plot_fit2 = True
if plot_fit2:
    plot_fit2_grad = 1
    plot_fit2_start = 1016 # Startpunkt der Fit-Kurve datenpunkte Array
    plot_fit2_end = 1552
    plot_fit2_xdata = t
    plot_fit2_ydata = T
    plot_fit2_extend_start = -50
    plot_fit2_extend_end = 0
...
if plot_fit == True:
    generate_fit_curve_data(plot_xdata, fit_grad, None, None, berechnete_ergebnisse, 'g--', 'g_', berechnete_fehler)
if plot_fit1 == True:
    coeff_1 = generate_fit_curve_data( plot_fit1_xdata, plot_fit1_grad, plot_fit1_start, plot_fit1_end, plot_fit1_ydata, 'r--', 'r_', berechnete_fehler, extend_start=plot_fit1_extend_start, extend_end=plot_fit1_extend_end)
if plot_fit2 == True:
    coeff_2 = generate_fit_curve_data( plot_fit2_xdata, plot_fit2_grad, plot_fit2_start, plot_fit2_end, plot_fit2_ydata, 'k--', 'k_', berechnete_fehler, extend_start=plot_fit2_extend_start, extend_end=plot_fit2_extend_end)
# Plot-Einstellungen
plt.plot([216, 216], [70, 44], color="green", linestyle="-.", label="Übergang Mittel bei 216s")
plt.scatter(216,69.37384525, marker="x", color="purple", s=100, label=r"Schnittpunkt bei 69.4C$^{\circ}$", zorder=5)
plt.scatter(216, 44.18841725, marker="x", color="purple", s=100, label=r"Schnittpunkt bei 44.2C$^{\circ}$", zorder=5)
