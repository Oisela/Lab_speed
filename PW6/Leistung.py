...
ergebnis_einheit = 'W'  # Einheit des Ergebnisses
ergebnis_name = r'Leistung $P$'  # Name des Ergebnisses
...
messwerte = [
    [U[i], I[i]] for i in range(anzahl_messungen) 
]
...
plot_xlabel = r'R in Ohm'                 # Beschriftung der x-Achse
plot_ylabel = r'P in Watt'      # Beschriftung der y-Achse
plot_titel  = r'' # Titel des Plots
plot_error = False                # Fehlerbalken im Plot anzeigen
plot_xdata = R  # x-Achse: wenn Messungezahal, dann : Messungsnummern
...
if plot_error:
    ...
    plt.plot(plot_xdata, plot_ydata, 'bo', label='Messwerte 1')
    plt.plot(plot_xdata, P_2, 'go', label='Messwerte 2 ')
