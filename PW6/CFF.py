...
ergebnis_name = r'CFF '  # Name des Ergebnisses
...
P_max, I_KS, U_LL = sp.symbols('P_max I_KS U_LL')

# Formel mit SymPy-Funktionen definieren
formel = P_max / (I_KS * U_LL)

# Definiere die Variablen als String (Reihenfolge muss mit den Daten übereinstimmen)
variablen = 'P_max I_KS U_LL'

# Konstanten (für alle Messungen gleich)
U_LL = np.array([3.26,3.33])  # V
I_KS = np.array([0.012434,0.02838])  # A
P_max  = np.array([0.02769, 0.0759])  # W

error_P_max = np.array([0.00061, 0.0014])  # Fehler bei 'P'
error_I_KS = np.array([0.000020,0.00029])  # Fehler bei 'I_KS'
error_U_LL = np.array([0.52,0.42])  # Fehler bei 'U_LL'
...
messwerte = [
    [P_max[i], I_KS[i], U_LL[i]] for i in range(anzahl_messungen) 
]
# Fehler für jede Variable
fehler = [
    [error_P_max[i], error_I_KS[i], error_U_LL[i]] for i in range(anzahl_messungen)
]
