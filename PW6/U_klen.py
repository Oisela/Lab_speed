...
# Datei leeren und Datum und Uhrzeit schreiben
with open(dateipfad, 'w', encoding='utf-8') as f:
    now = datetime.now()
    datum_uhrzeit = now.strftime("%Y-%m-%d %H:%M:%S")
    f.write(f"Datum und Uhrzeit: {datum_uhrzeit}\n\n")
...
def speichere_ergebnisse(dateipfad, messungsnummern, ergebnisse, fehler, variablen_werte,
    ...
    with open(dateipfad, 'a', encoding='utf-8') as f:
        ...
        if len(ergebnisse) > 1:
            ...
            for i, n, dn in zip(messungsnummern, ergebnisse, fehler):
                f.write(f"\t \t \hline {i} & ${n:.4g} \pm {dn:.4g}$ \\\ \n")
            f.write("\t \t \\hline \n")
            f.write("\t \\end{tabular} \n")
...
anzahl_messungen = 12  # Kann leicht angepasst werden
skalierung =   1  # Skalierungsfaktor für die Ergebnisse (z.B. 1000 für mPa·s)
ergebnis_einheit = 'A'  # Einheit des Ergebnisses
...
for i in range(anzahl_messungen):
    if U_KL[i] > 60:
        error_U.append(unsicherheiten_Multimeter(U_KL[i], 0.01, 0.1, 3))
    else:
        error_U.append(unsicherheiten_Multimeter(U_KL[i], 0.01, 0.01, 3))
text = ""
for i in error_U:
    text += str(i) + "\n"
whrite_to_file(dateipfad, text)
...
error_I = []
for i in range(anzahl_messungen):
    error_I.append(unsicherheiten_Multimeter(I[i], 0.005, 0.001, 3))
text = ""
text += "\n"
for i in error_I:
    text += str(i) + "\n"
...
messwerte = [
    [U_KL[i], I[i]] for i in range(anzahl_messungen) 
]
# Fehler für jede Variable
fehler = [
    [error_U[i], error_I[i]] for i in range(anzahl_messungen)
]
...
plot_xlabel = r'I in Ampere'                 # Beschriftung der x-Achse
plot_ylabel = r'$U_{KL}$ in Volt'      # Beschriftung der y-Achse
plot_titel  = r'' # Titel des Plots
plot_error = False                # Fehlerbalken im Plot anzeigen
plot_xdata = U_KL # x-Achse: wenn Messungezahal, dann : Messungsnummern
...
if plot_error:
    plt.errorbar(plot_xdata, plot_ydata, yerr=berechnete_fehler, xerr=error_I,  fmt='o', label='Messwerte', capsize=5)
else:
...
if plot_fit == True:
    generate_fit_curve_data(max_signifikante_stellen, 
                            plot_xdata, plot_fit, fit_grad, None, None, whrite_to_file, dateipfad, plot_ydata, 'g--', 'g_', berechnete_fehler)
if plot_fit1 == True:
