...
ergebnis_name = 'Spaltenbreite'  # Name des Ergebnisses
Signifikante_Stellen = 3    # Anzahl der signifikanten Stellen
...
fehler_a = 9.7873e-07  # Fehler bei 'a'
fehler_N = 1  # Fehler bei 'N'
...
messwerte = [
    [N, a] for i in range(anzahl_messungen) 
]
# Fehler für jede Variable
fehler = [
    [fehler_N, fehler_a] for i in range(anzahl_messungen)
]
...
def speichere_ergebnisse(dateiname, messungsnummern, ergebnisse, fehler, variablen_werte,
    ...
    try:
        ...
        with open(dateipfad, 'w', encoding='utf-8') as f:
            ...
            # Header schreiben
            f.write(f"{'Messung':<10}{f'{ergebnis_name} ({ergebnis_einheit})':<40}{f'Fehler ({ergebnis_einheit})':<20}\n")

            # Ergebnisse schreiben
            for i, n, dn in zip(messungsnummern, ergebnisse, fehler):
                f.write(f"{i:<10}{n:<40}{dn:<20}\n")
            ...
            if len(ergebnisse) > 1:
                ...
                for i, n, dn in zip(messungsnummern, ergebnisse, fehler):
                    ...
                    f.write(f"\t \t \hline {i} & ${n:.{Signifikante_Stellen}g} \pm {dn}$ \\\ \n")
                f.write("\t \t \\hline \n")
                f.write("\t \\end{tabular} \n")
