...
def latex_ergebnisse(dateipfad, ergebnisse, fehler):
    with open(dateipfad, 'a', encoding='utf-8') as f:
        ...
        f.write("\\end{align*} \n")
        f.write("\n")
# Test if it works with the new function
def cvsimport(file, delimiter, skip_header):
    ...
    with open(file, newline='') as csvfile:
        ...
        data = list(reader)
    data_x = [float(daten[i][0]) for i in range(anzahl_messungen)]
    data_y = [float(daten[i][1]) for i in range(anzahl_messungen)]
...
korrelierte_unsicherheitn  = True # Korrelierte Unsicherheiten
Gewichteter_mean_error = False
anzahl_messungen = 2  # Kann leicht angepasst werden
skalierung =   1  # Skalierungsfaktor für die Ergebnisse (z.B. 1000 für mPa·s)
ergebnis_einheit = r'$m^2$'  # Einheit des Ergebnisses
ergebnis_name = r'Fläche'  # Name des Ergebnisses
...
L = np.array([9.7, 9.7], dtype=float)  # Längenmessungen
B = np.array([3.9] * anzahl_messungen, dtype=float)  # Breite als konstantes Array

# Fehler definieren
error_L = np.array([0.05] * anzahl_messungen, dtype=float)
error_B = np.array([0.05] * anzahl_messungen, dtype=float)
# From 
...
messwerte = [
    [L[i], B[i]] for i in range(anzahl_messungen) 
]
# Fehler für jede Variable
fehler = [
    [error_L[i], error_B[i]] for i in range(anzahl_messungen)
]
...
if messungsnummern is not None and berechnete_ergebnisse is not None and berechnete_fehler is not None:
    speichere_ergebnisse(dateipfad, messungsnummern, berechnete_ergebnisse, berechnete_fehler, variablen_werte)
else:
    print("Error: One or more required variables (messungsnummern, berechnete_ergebnisse, berechnete_fehler) are None.")
