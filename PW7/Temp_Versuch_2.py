...
def calculate_scaled_results_error(anzahl_messungen, skalierung, formel, variablen, messwerte, fehler ):
    for i in range(anzahl_messungen):
        ...
        if korrelierte_unsicherheitn:
            ergebnis, gesamtfehler = fehlerfortpflanzung_korrelierte_auto(formel, variablen, messwerte[i], fehler[i])
        else:
...
def fehlerfortpflanzung_korrelierte_auto(formel_str, variablen_str, messwerte, fehler, kovarianz_matrix=None, korrelierte_unsicherheiten=False):
    """
    Berechnet die Fehlerfortpflanzung nach der Gauß'schen Methode mit oder ohne korrelierte Unsicherheiten.

    Args:
        formel_str (str): Die mathematische Formel als String.
        variablen_str (str): Die Variablen in der Formel als String, getrennt durch Leerzeichen.
        messwerte (list of lists): Liste der Messwerte für jede Messung.
        fehler (list of lists): Liste der Fehler für jede Variable in jeder Messung.
        kovarianz_matrix (np.array): Kovarianzmatrix der Variablen (nur bei korrelierten Unsicherheiten).
        korrelierte_unsicherheiten (bool): Ob korrelierte Unsicherheiten berücksichtigt werden sollen.

    Returns:
        list of tuples: Liste von (Ergebnis, Gesamtfehler) für jede Messung.
    """
    # Symbole erstellen
    variablen = sp.symbols(variablen_str)
    formel = sp.sympify(formel_str)

    # Ergebnisse und Fehler für jede Messung
    ergebnisse = []

    # Schleife über alle Messungen
    for i in range(len(messwerte)):
        # Aktuelle Messung und Fehler
        aktuelle_messung = messwerte[i]
        aktuelle_fehler = fehler[i]

        # Werte-Dictionary erstellen
        werte_dict = dict(zip(variablen, aktuelle_messung))

        # Funktionswert berechnen
        funktionswert = float(formel.evalf(subs=werte_dict))

        # Partielle Ableitungen berechnen
        ableitungen = [float(formel.diff(var).evalf(subs=werte_dict)) for var in variablen]

        # Fehlerfortpflanzung berechnen
        quadratischer_fehler = sum((dfdx * delta)**2 for dfdx, delta in zip(ableitungen, aktuelle_fehler))

        # Korrelationsterm hinzufügen, wenn korrelierte Unsicherheiten aktiviert sind
        if korrelierte_unsicherheiten and kovarianz_matrix is not None:
            kovarianz = np.array(kovarianz_matrix)
            for m in range(len(variablen)):
                for n in range(len(variablen)):
                    if m != n:  # Nur Off-Diagonale
                        quadratischer_fehler += 2 * ableitungen[m] * ableitungen[n] * kovarianz[m, n]

        # Gesamtfehler berechnen
        gesamtfehler = np.sqrt(quadratischer_fehler)
        ergebnisse.append((funktionswert, gesamtfehler))
...
calculate = False # Berechnung der Ergebnisse
korrelierte_unsicherheitn  = False # Korrelierte Unsicherheiten
Gewichteter_mean_error = False
anzahl_messungen = 146  # Kann leicht angepasst werden
skalierung =   1  # Skalierungsfaktor für die Ergebnisse (z.B. 1000 für mPa·s)
ergebnis_einheit = 'celsius'  # Einheit des Ergebnisses
...
delta_x, T = sp.symbols('delta_x T')
...
daten = cvsimport(os.path.join(os.path.dirname(__file__), '2530.csv'), ',', True)


delta_x = [float(daten[i][0]) for i in range(anzahl_messungen)]
T = [float(daten[i][1]) for i in range(anzahl_messungen)]
print(delta_x)
error_delta_x = [0] * anzahl_messungen 
error_T = [0] * anzahl_messungen
...
messwerte = [
    [delta_x[i], T[i]] for i in range(anzahl_messungen) 
]
# Fehler für jede Variable
fehler = [
    [error_delta_x[i], error_T[i]] for i in range(anzahl_messungen)
]
...
plot_xlabel = r'$\Delta x$ in Pixel'                 # Beschriftung der x-Achse
plot_ylabel = r'Temperatur in C°'      # Beschriftung der y-Achse
plot_titel  = r'' # Titel des Plots
plot_error = False                # Fehlerbalken im Plot anzeigen
plot_xdata = delta_x  # x-Achse: wenn Messungezahal, dann : Messungsnummern if False dann ergebnisse
plot_ydata = T  # y-Achse: Messwerte if False dann ergebnisse
plot_xerror = None
plot_yerror = None
...
speichere_ergebnisse(dateipfad, messungsnummern,berechnete_ergebnisse, berechnete_fehler, variablen_werte)
