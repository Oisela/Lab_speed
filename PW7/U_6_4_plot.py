...
def calculate_scaled_results_error(fehlerfortpflanzung, anzahl_messungen, skalierung, formel, variablen, messwerte, fehler ):
...
anzahl_messungen = 8  # Kann leicht angepasst werden
...
error_r2 = [0.01] * anzahl_messungen

### R in meter
##r = r * 0.01
### Fehler 
error_r = [0.01] * anzahl_messungen
...
error_V = []
for i in range(anzahl_messungen):
    error_V.append(unsicherheiten_Multimeter(V_Aus[i], 0.008, 0.1, 3))

error_V2 = []
for i in range(anzahl_messungen):
    error_V2.append(unsicherheiten_Multimeter(V2_Aus[i], 0.008, 0.1, 3))
...
plot_xdata = R  # x-Achse: wenn Messungezahal, dann : Messungsnummern if False dann ergebnisse
plot_ydata = V_Aus  # y-Achse: Messwerte if False dann ergebnisse
plot_xerror = None
plot_yerror = None
...
Nr2_plot_ydata = V2_Aus     # if False dann ergebnisse
Nr2_plot_xdata = R2         # if False dann Messungsnummern
...
if calculate:
    berechnete_ergebnisse, berechnete_fehler = calculate_scaled_results_error(fehlerfortpflanzung, anzahl_messungen, skalierung, formel, variablen, messwerte, fehler)
else: 
    berechnete_ergebnisse = plot_ydata
if Nr2_calculate:
    Nr2_berechnete_ergebnisse, Nr2_berechnete_fehler = calculate_scaled_results_error(fehlerfortpflanzung, Nr2_anzahl_messungen, Nr2_skalierung, formel, variablen, Nr2_messwerte, Nr2_fehler )
else:
    Nr2_berechnete_ergebnisse = Nr2_plot_ydata
if Nr3_calculate:
    Nr3_berechnete_ergebnisse, Nr3_berechnete_fehler = calculate_scaled_results_error(fehlerfortpflanzung, Nr3_anzahl_messungen, Nr3_skalierung, formel, variablen, Nr3_messwerte, Nr3_fehler)
else:
...
#DEl
print( len(plot_xdata), len(berechnete_ergebnisse), len(berechnete_fehler))
print( len(Nr2_plot_xdata), len(Nr2_plot_ydata), len(Nr2_berechnete_fehler))
