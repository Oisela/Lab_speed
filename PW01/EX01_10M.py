...
plot_titel  = 'Viskosität von Wasser'
...
delta_h = 0.1965  # Höhenunterschied in Metern
...
messwerte = [
    [a, roh, delta_h, delta_t_values[i]] for i in range(anzahl_messungen)
]
...
def fehlerfortpflanzung(formel_str, variablen_str, messwerte, fehler):
    ...
    for dfdx, delta in zip(ableitungen, fehler):
        ...
        # Ausgabe der Berechnungen Schritt für Schritt
        print(f"Ableitung nach {dfdx}: {dfdx_wert}")
        print(f"Fehler {delta}: {dfdx_wert * delta}")
        print(f"Quadratischer Fehler: {(dfdx_wert * delta)**2}")
        print("-" * 40)
        
    gesamtfehler = np.sqrt(quadratischer_fehler)
    
    # Ausgabe der Berechnungen
    print(f"Berechnung für Messwerte: {messwerte}")
    print(f"Formelwert: {formel_wert}")
    print(f"Gesamtfehler: {gesamtfehler}")
    print("=" * 40)
