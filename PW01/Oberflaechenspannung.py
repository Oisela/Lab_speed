...
ergebnis_einheit = 'N/m'  # Einheit des Ergebnisses
ergebnis_name = 'Oberflächenspannung'  # Name des Ergebnisses

#Label Plot 
plot_xlabel = 'Messungen'
plot_ylabel = 'Oberflächenspannungen (N/m)'
plot_titel  = 'Oberflächenspannungen von destilliertem Wasser'
...
a = 0.5 # Angepasster Wert für 'a' in m²
m = 1.504


# Absolute Fehler der Konstanten
fehler_a = 0 #  +- 0.8e-7 m² Fehler bei 'a'
fehler_D = 0.02e-3 # +- 0.1e-3 m Fehler bei 'D'


# Messwerte für 'delta_t' Kraftmessungen in Newton
x_values = [14, 14, 14 , 14, 13.5, 13.8, 13.8, 13.8, 13.8, 13]
fehler_x_values = [0.5] * anzahl_messungen  # Absoluter Fehler von 0.4 Sekunden
...
variablen_werte = {
    ...
    'Kraft (N)': x_values,
    'fehler_D (m)': fehler_D,
    'fehler_Kraft (N)': fehler_x_values
}

# Werte für jede Variable
messwerte = [
    [a, D, x_values[i]] for i in range(anzahl_messungen)
]

# Fehler für jede Variable
fehler = [
    [fehler_a, fehler_D, fehler_x_values[i]] for i in range(anzahl_messungen)
]
...
# Was plotet X-Achse
x_plot_ein = messungsnummern
# Variablen für die Y-Achse Anfangs- und Endwert "None" für automatische Skalierung
y_achse_start = 10
y_achse_ende = 15
...
# Y-Achse skalieren, wenn Werte gesetzt sind
if y_achse_start is not None and y_achse_ende is not None:
    plt.ylim(y_achse_start, y_achse_ende)
