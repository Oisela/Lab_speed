...
# Import OOP class data_set
from oop.data_set import data_set
...
# Define your measurement values dictionary


aussen_luftdruck = data_set("p_{aussen}", 780, 5, "mmHg", output_filepath)
zimmertemperatur = data_set("\\vartheta_{Raum}", 23, 1, "°C", output_filepath)
sättigung_wasser = data_set("p_w", 21.1, 0, "mmHg", output_filepath)
Volumen = data_set("V", 49, 0.2, "cm^3", output_filepath)
höhe_wasser = data_set("h", 37.5, 2, "cm", output_filepath)
...
FORMULA = "780 -  "         # Formula to calculate results
...
PLOT_TITLE = "Example Physical Measurement"  # Plot title (None = auto-generate)
PLOT_XLABEL = "Distance (m)"  # X-axis label (None = auto-generate)
PLOT_YLABEL = "Velocity (m/s)"  # Y-axis label (None = auto-generate)
...
FIT_LABEL = "Linear Fit"  # Label for fit line
...
def main():
    ...
    if PLOT_ENABLED:
        ...
        plot_data_with_errors(
            ...
            style=PLOT_STYLE,
            label=f"Main dataset: {main_data['file_name']}"
        )
