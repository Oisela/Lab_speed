...
ergebnis_name = 'Brechnungsindex einer Konkavenlinse'  # Name des Ergebnisses
...
fehler_e = 0.005  # Absoluter Fehler von 0.005 Meter


# Messwerte 
d = np.array([0.517, 0.512, 0.510, 0.512, 0.516])  # Meter

# Fehler Messerte
fehler_d = 0.005  # 

Mehrfachmessungen = True # Wenn True, dann wird der Mittelwert der Messungen berechnet
delta_freedom = 1 # Freiheitsgrad für die Standardabweichung

if Mehrfachmessungen:
    Len_d = len(d)
    Vertrausenbereich_d = np.std(d, ddof=delta_freedom) / np.sqrt(Len_d)
    fehler_d = np.sqrt(Vertrausenbereich_d**2 + fehler_d**2) 

    d = np.mean(d)

    print(f'Mittelwert d: {d:.3f} m')
    print(f'feher d: {fehler_d:.3g} m')
...
# Korrigierte Definitionen
## Achtung Reihenfolge beachten ##
messwerte = [
    [e, d] for i in range(anzahl_messungen)  ## Achtung Reihenfolge wie in der Formel
]
## Achtung Reihenfolge beachten ##
fehler = [
    [fehler_e, fehler_d] for i in range(anzahl_messungen)
]
