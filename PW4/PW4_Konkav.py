...
ergebnis_name = 'Winkel'  # Name des Ergebnisses
...
formel = sp.atan(d/(2*L))

# Definiere die Variablen als String (Reihenfolge muss mit den Daten übereinstimmen)
variablen = 'd L'

# Konstanten (für alle Messungen gleich)
L = 1.256   # Abstand zwischen den Spalten in m

# Absolute Fehler der Konstanten
fehler_l = 0.003  # Absoluter Fehler von 0.005 Meter


d = np.array([10, 20.5, 33, 43, 54, 66])  # Abstand zwischen den Spalten in m

d = d / 1000  # Umrechnung in Meter

fehler_d = [0.003] * anzahl_messungen # Absoluter Fehler von 0.0005 Meter
...
messwerte = [
    [L, d[i]] for i in range(anzahl_messungen)  ## Achtung Reihenfolge wie in der Formel
]
## Achtung Reihenfolge beachten ##
fehler = [
    [fehler_l, fehler_d[i]] for i in range(anzahl_messungen)
]
