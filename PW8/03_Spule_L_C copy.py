...
error_U_R = []
for i in range(anzahl_messungen):
    error_L = unsicherheiten_Multimeter(U_L[i], 0.01, 0.001, 3)
    error_X = unsicherheiten_Multimeter(U_R[i], 0.01, 0.001, 3)
    error_U_L.append(error_L)
...
messwerte = [
    [R, omega[i], U_L[i], U_R[i]] for i in range(anzahl_messungen) 
]
# Fehler für jede Variable
fehler = [
    [error_R, error_omega[i], error_U_L[i], error_U_R[i]] for i in range(anzahl_messungen)
]
