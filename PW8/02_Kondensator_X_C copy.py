...
def generate_fit_curve_data( plot_xdata, plot_fit2, plot_fit2_grad, plot_fit2_start, plot_fit2_end, plot_ydata, linestyle, dotstyle, y_errors=None):
    ...
    if fit_funktion == True:
        # Fit-Kurve berechnen
        coefficients, cov_matrix = curve_fit(fit_function, plotX_xdata, plotX_ydata, sigma=y_errors)
        fit2_x = np.linspace(min(plotX_xdata), max(plotX_xdata), 1000)
        fit2_y = fit_function(fit2_x, *coefficients)
        plt.plot(fit2_x, fit2_y, linestyle, label=f'Curve-Fit {fit_function_name}')
        residuals = plotX_ydata - fit_function(plotX_xdata, *coefficients)
...
ergebnis_einheit = r'\Omega'  # Einheit des Ergebnisses no latex needed
ergebnis_name = r'Wiederstand'  # Name des Ergebnisses
...
error_U_R = []
for i in range(anzahl_messungen):
    error_C = unsicherheiten_Multimeter(U_c[i], 0.01, 0.001, 3)
    error_X = unsicherheiten_Multimeter(U_R[i], 0.01, 0.001, 3)
    error_U_C.append(error_C)
    error_U_R.append(error_X)
# From 
...
messwerte = [
    [R, omega[i], U_c[i], U_R[i]] for i in range(anzahl_messungen) 
]
# Fehler für jede Variable
fehler = [
    [error_R, error_omega[i], error_U_C[i], error_U_R[i]] for i in range(anzahl_messungen)
]
...
plot_ylabel = r'$X_c$ in $\Omega$ '      # Beschriftung der y-Achse
