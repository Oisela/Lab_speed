...
def sience_error(number, significant_digits):
    if number == 0:
        ...
        if len(original_number_str) - 1 < formetted_length:
            ...
            if float(original_number_str[formetted_length]) < 5 and float(original_number_str[formetted_length]) != 0:
...
ergebnis_einheit = r'$s^{-1}$'  # Einheit des Ergebnisses
ergebnis_name = r'Kreisfrequenz'  # Name des Ergebnisses
...
error_v = np.array([2] * anzahl_messungen)
...
messwerte = [
    [v[i], a] for i in range(anzahl_messungen) 
]
# Fehler für jede Variable
fehler = [
    [error_v[i], error_a] for i in range(anzahl_messungen)
]
